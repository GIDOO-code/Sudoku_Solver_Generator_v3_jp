...
//▼▼▼ 開発中 ▼▼▼
    public partial class GroupedLinkGen: AnalyzerBaseV2{
		...
		private string _krfMsg;
        public bool KrakenFish( ){
			Prepare();
			for( int sz=2; sz<5; sz++ ){
				for( int no=0; no<9; no++ ){
					if( ExtKrFishSub(sz,no,27,FinnedF:false) ) return true;
				}
            }
            return false;
        }
        public bool KrakenFinnedFish( ){
			Prepare();
			for( int sz=2; sz<7; sz++ ){
				for( int no=0; no<9; no++ ){
					if( ExtKrFishSub(sz,no,27,FinnedF:true) )  return true;
				}
            }
            return false;
        }
                ...
                foreach( var Cov in FMan.IEGet_CoverSet(Bas,CoverSel,FinnedF) ){  //CoverSet生成  
                    ...
                    if( FinnedF!=FinB81.IsZero() ){
						...
						foreach( var Hb in Bas.HouseB.IEGet_BtoNo(27) ){
							...
							if(FinnedF) E|=FinB81;	//■Finあり
							if( E.IsZero() )  continue;

							foreach( var P in pBDL.Where(p=> !UsedB.IsHit(p.rc)) ){
								foreach( var noZ in P.FreeB.IEGet_BtoNo() ){
									int noZb=(1<<noZ);
									USuperLink USLK=pSprLKsMan.get_L2SprLK(P.rc,noZ,CorrectF:false,DevelopB:false);//#####
										//WriteLine( $" USuperLink rc:{P.rc} no:{noZ+1}" );
									if( !USLK.SolFond ) continue;
									Bit81 Ef = E - USLK.Qfalse[no];
									if( !Ef.IsZero() )  continue;

									//正確な分析(結果報告生成のために必要
									USLK=pSprLKsMan.get_L2SprLK(P.rc,noZ,CorrectF:true,DevelopB:false);//#####
									Ef = E - USLK.Qfalse[no];
									if( !Ef.IsZero() )  continue;
									P.CancelB|=noZb;
									SolCode=2;
					...
					//	LSolFond:
							if(SolCode>0){
								if(SolInfoDsp) extRes = krfSolMsg;
        ...
        private void __Dev_PutDBL( string dir, string fName, bool append ){
