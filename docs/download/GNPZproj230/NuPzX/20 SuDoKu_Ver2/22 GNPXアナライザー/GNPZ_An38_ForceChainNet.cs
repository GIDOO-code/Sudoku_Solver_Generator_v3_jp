    ...
    public partial class GroupedLinkGen: AnalyzerBaseV2{
        ...
        public bool ForceChain_Contradiction( ){
			...
			foreach( var P in pBDL.Where(p=>p.No==0) ){
				foreach( var no in P.FreeB.IEGet_BtoNo() ){
					int noB=(1<<no);
                    USuperLink USLK=pSprLKsMan.Eval_SuperLinkChain(P.rc,no,Gbreak:true, DevelopB:false);	//218
					if( !USLK.SolFond ){
						P.CancelB |= noB;
		...
		public bool ForceChainNetDevelop( ){
			...
			foreach( var P in pBDL.Where(p=>p.No==0) ){
				foreach( var no in P.FreeB.IEGet_BtoNo() ){
					int noB=(1<<no);
                    USuperLink USLK=pSprLKsMan.Eval_SuperLinkChain(P.rc,no,Gbreak:false, DevelopB:false);	//218
					if( USLK.SolFond ){
						GLC[no].BPSet(P.rc);
