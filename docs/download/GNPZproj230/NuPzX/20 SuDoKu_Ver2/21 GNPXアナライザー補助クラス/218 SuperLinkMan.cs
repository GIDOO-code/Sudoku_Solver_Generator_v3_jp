...
namespace GNPZ_sdk {
    public partial class SuperLinkMan{
		...
		public CellLinkMan     rCeLKMan;	 //C-C
		...
		public GroupedLinkMan     rGLKMan;	 //C-C
...
//		private bool DevelopB=false;  //true; false;
        ...
        public void Initialize(){ //37
			CeLKMan.Initialize();
			GLKMan.Initialize();
			ALSMan.Initialize();
		}

        public void PrepareSuperLinkMan( bool AllF=false ){	//37  ■SuperLKを使う箇所に配置
            CeLKMan.PrepareCellLink(1+2);    //strongLink,weakLink生成
            if( AllF || GNPXpzl.GMthdOption["GroupedCells"]=="1" )  GLKMan.Initialize();
            if( AllF || GNPXpzl.GMthdOption["ALS"]=="1" )           ALSMan.QSearch_AlsInnerLink();
		...
		public USuperLink Eval_SuperLinkChain( int rc0, int no0, bool Gbreak, bool DevelopB ){  //37
																	//Gbreak True:falseを発見したら中断　1:続行
			try{	
				int dbX=0;
				...
				foreach( var GLKH in IEGet_SuperLinkFirst(P0,no0).Where(X=>X.type==W) ){	//.218
				...
				foreach( var nox in P0.FreeB.IEGet_BtoNo().Where(n=>(n!=no0)) ){
					Qfalse[nox].BPSet(rc0);   //その他の数字
				}

				foreach( var GLKH in IEGet_SuperLinkFirst(P0,no0).Where(X=>X.type==S) ){	//.218
					int nox=GLKH.no2;
				...
				while(rcQue.Count>0){
					...
					foreach( var GLKH in IEGet_SuperLink(R) ){		//.218 36
        ...
        public IEnumerable<GroupedLink> IEGet_SuperLinkFirst( UCell UC, int no ){	//.218 36 subAn
            ...
            if( GNPXpzl.GMthdOption["GroupedCells"]=="1" ){
        ...
        public IEnumerable<GroupedLink> IEGet_SuperLink( GroupedLink GLKpre ){	//.218 36
            ...
            if( GNPXpzl.GMthdOption["ALS"]=="1" && ALSMan.AlsInnerLink!=null ){
                if( GLKpre.type==W ){
...
//C                            WriteLine( GP );
        ...
        public bool Check_SuperLinkSequence( GroupedLink GLKpre, GroupedLink GLKnxt ){ //.218 36
		...
		public bool GenMessage2( USuperLink USLK, UCell P, int no ){ //.218
			...
			string st = "";
			if(Pdes!=null)   st += "   "+_chainToString2(P,Pdes,-(no+1));	//.218
			Pdes=(GroupedLink)pchainDesLKF[P.rc,no];
			if(Pdes!=null){
				if(st.Length>2)  st += "\r";
				st += "   "+_chainToString2(P,Pdes,no+1);					//.218
			}
			USLK.stMsg=st;
			return true;
		}
		public string _GenMessage2true( USuperLink USLK, UCell P, int no ){ //38
			GroupedLink Pdes=(GroupedLink)USLK.chainDesLKT[P.rc,no];
			string st = "";
			if(Pdes!=null)   st += "   "+_chainToString2(P,Pdes,-(no+1));	//.218
			return st;
		}
		public string _GenMessage2false( USuperLink USLK, UCell P, int no ){ //38
			GroupedLink Pdes=(GroupedLink)USLK.chainDesLKF[P.rc,no];
			string st = "";
			if(Pdes!=null)   st += "   "+_chainToString2(P,Pdes,-(no+1));	//.218
			return st;
		}
		private string _chainToString2( UCell U, GroupedLink Gdes, int noRem ){ //.218
            string st="";
