...
namespace GNPZ_sdk {
    ...
    #region superlinkChain
            ...
            if(DevelopB){
                ...
                foreach( var P in pBDL ){
                    ...
                    foreach( var no in P.FreeB.IEGet_BtoNo() ){
						...
						string st= _GenMessage( USLK, P, no);	//.219
                        if(st.Length>4){
							WriteLine(st);
							stMsg += st;
						}
		...
		private string _GenMessage( USuperLink USLK, UCell P, int no ){		//.219
			string st="";
            var pQtrue =USLK.Qtrue;
            var pQfalse=USLK.Qfalse;
            var pchainDesLKT=USLK.chainDesLKT;
		...
		private string _chainToString( UCell U, GroupedLink Gdes, int noRem ){ //.219
            string st="";
