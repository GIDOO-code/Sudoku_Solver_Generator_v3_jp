...
namespace GNPZ_sdk {
    public class ALSLinkMan{
        ...
        public List<ALSLink>        AlsInnerLink;   //ALS内リンク
        public List<LinkCellAls>[]  LinkCeAlsLst;   //Cell->ALS リンク
        public bool                 ALS2ALS_Link;   //ALS ->ALS リンクはUALSに保持
        ...
        public int PrepareALSLinkMan( int nPls ){ //QALS_Search
            ...
            int ALSSizeMax = GNPXpzl.ALSSizeMax;
            ...
            for( int nn=1; nn<=nPls; nn++ ){
                for( int tf=0; tf<27; tf++ ){
                    ...
                    szMax = Min(szMax,ALSSizeMax);       //【変更】ALSサイズ最大値を制限
        ...
        public int  Get_AlsAlsRcc( UALS UA, UALS UB ){           
            ...
            //【定義】rcbDir |= ( (1<<(P.b+18)) | (1<<(P.c+9)) | (1<<(P.r)) );
        ...
        //1つのALS内のリンク(1数字が除外されALSがLockしたとき、他の数字が1house内にある）
        public void QSearch_AlsInnerLink(){
            ...
            foreach( var Pals in ALSLst.Where(p=>(p.Size>=2&&p.singly)) ){
                ...
                Permutation prm=new Permutation(noLst.Count,2);//順列で両方向からのリンクを求める
                while( prm.Successor(2) ){
                    int noS = noLst[prm.Pnum[0]];
                    int noD = noLst[prm.Pnum[1]];

                    UGrCells GS = Pals.SelectNoCells(noS);//数字noSを持つALSセル群
                    UGrCells GD = Pals.SelectNoCells(noD);//数字noDを持つALSセル群
